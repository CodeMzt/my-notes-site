...
参考[3.11 图像处理 API 手册 | 立创开发板技术文档中心](https://wiki.lckfb.com/zh-hans/lushan-pi-k230/api/openmv/image_module_api.html)
...
在 **OpenMV** (通常是運行 MicroPython) 中，**`gc.collect()`** 是用於執行 **垃圾回收 (Garbage Collection, GC)** 的命令。

#### **垃圾回收 (GC)**

垃圾回收是一种自动内存管理机制。在 MicroPython（OpenMV 使用的语言）中，当你创建对象（例如图像、列表、变量等）时，它们会占用内存。

- 当一个对象**不再被任何程序代码引用或使用**时，它就变成了“垃圾”内存。
- **`gc.collect()`** 的作用就是**找出并释放**这些不再使用的内存空间，让这些空间可以被新的对象重新使用。

------

#### **为什么要使用 `gc.collect()`?**

OpenMV 是一个**嵌入式系统**，其内存资源通常非常有限。虽然 MicroPython 会自动进行垃圾回收，但在处理大型对象（如图像帧）或进行大量内存分配操作时，内存碎片化和内存不足的问题可能会出现。

- **手动触发** `gc.collect()` 可以**立即回收**那些未使用的内存。
- 这有助于**防止**程序在需要更多内存时因为**内存不足 (MemoryError)** 而崩溃。
- 它也有助于**减少内存碎片**，从而提高内存分配的效率。

#### **用法**

在 OpenMV 的 MicroPython 环境中，调用 `gc.collect()` 的方式非常简单：
...
在标准的 Python 中，`gc.collect()` 可以接受参数来控制回收的代数（年轻代、年老代等）。根据 MicroPython 的实现，`gc.collect()` 通常会执行一个**完整的回收**，尝试释放尽可能多的内存。

- **何时调用：** 最常见的是在处理完一个**大对象**（例如，处理完一帧图像后）或者在程序的**循环开始**时调用，以确保有足够的内存用于接下来的操作。
- **性能影响：** 垃圾回收操作本身需要消耗一定的处理器时间。因此，不应该在性能要求极高的代码段中频繁调用，而是应该在逻辑上合适且需要清理内存时调用。
...
```python
image.mean_pooled(x_div, y_div)1
```
