参见：[Sensor 模块 API 手册 — CanMV K230](https://www.kendryte.com/k230_canmv/zh/main/zh/api/mpp/K230_CanMV_Sensor模块API手册.html)
...
![](../../../photosource/k230-canmv-camera-top.png)
...
在图像处理应用中，用户通常需要首先创建一个 `Sensor` 对象。CanMV K230 软件可以自动检测内置的图像传感器，无需用户手动指定具体型号，只需设置传感器的最大输出分辨率和帧率。有关支持的图像传感器信息，请参见[图像传感器支持列表](Sensor.md#图像传感器支持列表)如果设定的分辨率或帧率与当前传感器的默认配置不符，系统会自动调整为最优配置，最终的配置可在日志中查看，例如 `use sensor 23, output 640x480@90`。
...
| crop      | 输出图像裁剪区域; 当输入为 `crop`=`True` 时，从画面中心自动裁切出合适的区域; 当输入 `crop` 为 `(crop_x, crop_y, crop_w, crop_h)` 时, `crop_x` 和 `crop_y` 为裁剪区域的左上角坐标，`crop_w` 和 `crop_h` 为裁剪区域的宽度和高度; | 输入      |
...
| 参数名称       | 描述                                    | 输入/输出 |
| ---------- | ------------------------------------- | ----- |
| pix_format | 输出图像格式，可选值见[数据结构描述](Sensor.md#数据结构描述) | 输入    |
| chn        | sensor 输出通道号                          | 输入    |
...
#### 模拟增益 (Analog Gain) 的含义

1. **作用**

模拟增益发生在信号被转换为数字数据之前。它的主要作用是**提高传感器的灵敏度**，特别是在**低光照环境**下。

- **增强信号：** 当光线较弱时，光电二极管产生的电信号微弱。模拟增益电路会将这个微弱的信号线性放大，使其更容易被后续的 ADC（模数转换器）识别和处理。

2. **单位**

增益值通常以 **分贝 (dB)** 为单位表示，因为它是一个放大比率的对数值。

- 例如，增益为 $10\text{ dB}$ 意味着信号的功率大约被放大了 $10$ 倍。

3. **与数字增益 (Digital Gain) 的区别**

| **特性**                  | **模拟增益 (Analog Gain - again())**                         | **数字增益 (Digital Gain - 通常是 dgain())**                 |
| ------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| **发生位置**              | **ADC 之前**（在模拟域）                                     | **ADC 之后**（在数字域）                                     |
| **对信噪比 (SNR) 的影响** | **能提高 SNR**。因为它放大了有效信号和噪声，但在放大噪声前，信号质量更好。 | **不能提高 SNR**。它只是将已经数字化的信号和噪声一起放大，通常会导致图像噪点更明显。 |
| **图像质量**              | **更优**。是低光照下首选的亮度提升方式。                     | **较差**。仅在模拟增益达到上限后作为补充手段。               |

4.  **调整策略（在低光照环境下）**

在图像处理中，通常会遵循以下策略来增加图像亮度：

1. **延长曝光时间 (Exposure Time)：** 这是提高图像质量的首选方法，但会受到物体移动（动态模糊）的限制。
2. **增加模拟增益 (`again`)：** 当曝光时间不能再延长时，增加模拟增益是下一个选择。
3. **增加数字增益 (`dgain`)：** 当模拟增益达到上限时，才考虑使用数字增益，以避免引入过多噪点。
...
| 返回值 | 描述                                                         |
| ------ | ------------------------------------------------------------ |
| bool   | 获取时返回当前状态：`True` 表示启用，`False` 表示关闭； 设置时返回操作结果：`True` 表示成功，`False` 表示失败。 |
...
| 返回值           | 描述                                                         |
| ---------------- | ------------------------------------------------------------ |
| focus_caps_tuple | 包含自动对焦功能及其范围的元组：`(isSupport, minPos, maxPos)`; `isSupport` 表示是否支持自动对焦功能，`minPos` 表示最小对焦位置，`maxPos` 表示最大对焦位置。 |
...
| 返回值    | 描述                                                         |
| --------- | ------------------------------------------------------------ |
| focus_pos | 获取时返回当前对焦位置； 设置时返回操作结果：`True` 表示成功，`False` 表示失败。 |
...
| ----------------- | ------------------------------------------------------------ |
| **数据类型**      | **`int` (整数)**                                             |
| **物理含义**      | 镜头马达的**步进计数**或**物理位移量**。                     |
| **范围**          | 由镜头模组的机械结构决定。通常 $0$ 到几千。                  |
| **最大值/最小值** | 通常代表镜头的**最近对焦距离**和**无穷远对焦**。             |
| **单位**          | 无标准单位（是电机步进单位），但代表焦平面与传感器平面的**距离**。 |
